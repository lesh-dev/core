--
-- PostgreSQL database dump
--

-- Dumped from database version 10.5 (Ubuntu 10.5-1.pgdg16.04+1)
-- Dumped by pg_dump version 10.5 (Ubuntu 10.5-1.pgdg16.04+1)

SET statement_timeout = 0;
SET lock_timeout = 0;
SET idle_in_transaction_session_timeout = 0;
SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;
SELECT pg_catalog.set_config('search_path', '', false);
SET check_function_bodies = false;
SET client_min_messages = warning;
SET row_security = off;


SET default_tablespace = '';

SET default_with_oids = false;

--
-- Name: contact; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.contact (
    id bigint GENERATED BY DEFAULT AS IDENTITY,
    person_id bigint,
    name text,
    value text
);


ALTER TABLE public.contact OWNER TO lesh;

--
-- Name: contestants; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.contestants (
    contestants_id bigint GENERATED BY DEFAULT AS IDENTITY,
    name text,
    mail text,
    phone text,
    parents text,
    address text,
    school text,
    level text,
    teacher_name text,
    work text,
    status text,
    contest_year text,
    fileexchange text
);


ALTER TABLE public.contestants OWNER TO lesh;

--
-- Name: course; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.course (
    course_id bigint GENERATED BY DEFAULT AS IDENTITY,
    course_title text,
    school_id bigint,
    course_cycle text,
    target_class text,
    course_desc text,
    course_comment text,
    course_created text,
    course_modified text,
    course_type text,
    course_area text,
    course_changedby text
);


ALTER TABLE public.course OWNER TO lesh;

--
-- Name: course_teachers; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.course_teachers (
    course_teachers_id bigint GENERATED BY DEFAULT AS IDENTITY,
    course_id bigint,
    course_teacher_id bigint,
    course_teachers_created text,
    course_teachers_modified text,
    course_teachers_changedby text
);


ALTER TABLE public.course_teachers OWNER TO lesh;

--
-- Name: department; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.department (
    department_id bigint GENERATED BY DEFAULT AS IDENTITY,
    department_title text,
    department_created text,
    department_modified text,
    department_changedby text
);


ALTER TABLE public.department OWNER TO lesh;

--
-- Name: exam; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.exam (
    exam_id bigint GENERATED BY DEFAULT AS IDENTITY,
    student_person_id bigint,
    course_id bigint,
    exam_status text,
    deadline_date text,
    exam_comment text,
    exam_created text,
    exam_modified text,
    exam_changedby text
);


ALTER TABLE public.exam OWNER TO lesh;

--
-- Name: notification; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.notification (
    notification_id bigint GENERATED BY DEFAULT AS IDENTITY,
    mail_group text,
    notification_text text,
    notification_html text
);


ALTER TABLE public.notification OWNER TO lesh;

--
-- Name: person; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.person (
    person_id bigint GENERATED BY DEFAULT AS IDENTITY,
    last_name text,
    first_name text,
    patronymic text,
    birth_date text,
    passport_data text,
    school text,
    school_city text,
    ank_class text,
    current_class text,
    phone text,
    cellular text,
    email text,
    skype text,
    social_profile text,
    is_teacher text,
    is_student text,
    favourites text,
    achievements text,
    hobby text,
    lesh_ref text,
    forest_1 text,
    forest_2 text,
    forest_3 text,
    tent_capacity text,
    tour_requisites text,
    anketa_status text,
    user_agent text,
    department_id bigint,
    person_created text,
    person_modified text,
    nick_name text,
    person_changedby text,
    teacher_fio text,
    parent_fio text,
    parent_phone text,
    parent_email text,
    school_period text,
    anketa_mode text
);


ALTER TABLE public.person OWNER TO lesh;

--
-- Name: person_comment; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.person_comment (
    person_comment_id bigint GENERATED BY DEFAULT AS IDENTITY,
    comment_text text,
    blamed_person_id bigint,
    school_id bigint,
    owner_login text,
    record_acl text,
    person_comment_created text,
    person_comment_modified text,
    person_comment_deleted text,
    person_comment_changedby text
);


ALTER TABLE public.person_comment OWNER TO lesh;

--
-- Name: person_school; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.person_school (
    person_school_id bigint GENERATED BY DEFAULT AS IDENTITY,
    member_person_id bigint,
    member_department_id bigint,
    school_id bigint,
    is_student text,
    is_teacher text,
    curatorship text,
    current_class text,
    courses_needed bigint,
    person_school_created text,
    person_school_modified text,
    curator_group text,
    person_school_comment text,
    person_school_changedby text,
    frm text,
    tll text,
    school_coords text
);


ALTER TABLE public.person_school OWNER TO lesh;

--
-- Name: problems; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.problems (
    problems_id bigint GENERATED BY DEFAULT AS IDENTITY,
    problem_name text,
    problem_html text,
    people text,
    criteria text,
    contest_year text
);


ALTER TABLE public.problems OWNER TO lesh;

--
-- Name: pss; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.pss (
    pss_id text NOT NULL,
    pss_value text,
    pss_created text,
    pss_modified text,
    pss_changedby text
);


ALTER TABLE public.pss OWNER TO lesh;

--
-- Name: school; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.school (
    school_id bigint GENERATED BY DEFAULT AS IDENTITY,
    school_title text,
    school_type text,
    school_date_start text,
    school_date_end text,
    school_created text,
    school_modified text,
    school_location text,
    school_changedby text,
    school_coords text
);


ALTER TABLE public.school OWNER TO lesh;

--
-- Name: solutions; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.solutions (
    solutions_id bigint GENERATED BY DEFAULT AS IDENTITY,
    problem_id text,
    contestant_id bigint,
    resolution_text text,
    resolution_author text,
    resolution_mark text,
    contest_year text
);


ALTER TABLE public.solutions OWNER TO lesh;

--
-- Name: submission; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.submission (
    submission_id bigint GENERATED BY DEFAULT AS IDENTITY,
    mail text,
    attachment text,
    fileexchange text,
    submission_timestamp text,
    sender text,
    contest_year text,
    replied text,
    processed text
);


ALTER TABLE public.submission OWNER TO lesh;

--
-- Name: users; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.users (
    id bigint GENERATED BY DEFAULT AS IDENTITY,
    social_id text,
    nickname text,
    email text
);


ALTER TABLE public.users OWNER TO lesh;

--
-- Name: xversion; Type: TABLE; Schema: public; Owner: lesh
--

CREATE TABLE public.xversion (
    db_version text
);


ALTER TABLE public.xversion OWNER TO lesh;

--
-- Name: school idx_25037_school_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.school
    ADD CONSTRAINT idx_25037_school_pkey PRIMARY KEY (school_id);


--
-- Name: contestants idx_25043_contestants_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.contestants
    ADD CONSTRAINT idx_25043_contestants_pkey PRIMARY KEY (contestants_id);


--
-- Name: problems idx_25049_problems_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.problems
    ADD CONSTRAINT idx_25049_problems_pkey PRIMARY KEY (problems_id);


--
-- Name: solutions idx_25055_solutions_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.solutions
    ADD CONSTRAINT idx_25055_solutions_pkey PRIMARY KEY (solutions_id);


--
-- Name: notification idx_25061_notification_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.notification
    ADD CONSTRAINT idx_25061_notification_pkey PRIMARY KEY (notification_id);


--
-- Name: course idx_25067_course_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.course
    ADD CONSTRAINT idx_25067_course_pkey PRIMARY KEY (course_id);


--
-- Name: course_teachers idx_25073_course_teachers_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.course_teachers
    ADD CONSTRAINT idx_25073_course_teachers_pkey PRIMARY KEY (course_teachers_id);


--
-- Name: department idx_25079_department_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.department
    ADD CONSTRAINT idx_25079_department_pkey PRIMARY KEY (department_id);


--
-- Name: person idx_25085_person_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person
    ADD CONSTRAINT idx_25085_person_pkey PRIMARY KEY (person_id);


--
-- Name: person_school idx_25091_person_school_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_school
    ADD CONSTRAINT idx_25091_person_school_pkey PRIMARY KEY (person_school_id);


--
-- Name: exam idx_25097_exam_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.exam
    ADD CONSTRAINT idx_25097_exam_pkey PRIMARY KEY (exam_id);


--
-- Name: submission idx_25103_submission_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.submission
    ADD CONSTRAINT idx_25103_submission_pkey PRIMARY KEY (submission_id);


--
-- Name: person_comment idx_25115_person_comment_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_comment
    ADD CONSTRAINT idx_25115_person_comment_pkey PRIMARY KEY (person_comment_id);


--
-- Name: pss idx_25121_sqlite_autoindex_pss_1; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.pss
    ADD CONSTRAINT idx_25121_sqlite_autoindex_pss_1 PRIMARY KEY (pss_id);


--
-- Name: contact idx_25127_contact_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.contact
    ADD CONSTRAINT idx_25127_contact_pkey PRIMARY KEY (id);


--
-- Name: users idx_25133_users_pkey; Type: CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT idx_25133_users_pkey PRIMARY KEY (id);


--
-- Name: idx_25133_sqlite_autoindex_users_1; Type: INDEX; Schema: public; Owner: lesh
--

CREATE UNIQUE INDEX idx_25133_sqlite_autoindex_users_1 ON public.users USING btree (social_id);


--
-- Name: contact contact_person_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.contact
    ADD CONSTRAINT contact_person_id_fkey FOREIGN KEY (person_id) REFERENCES public.person(person_id);


--
-- Name: course_teachers course_teachers_course_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.course_teachers
    ADD CONSTRAINT course_teachers_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.course(course_id);


--
-- Name: course_teachers course_teachers_course_teacher_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.course_teachers
    ADD CONSTRAINT course_teachers_course_teacher_id_fkey FOREIGN KEY (course_teacher_id) REFERENCES public.person(person_id);


--
-- Name: exam exam_course_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.exam
    ADD CONSTRAINT exam_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.course(course_id);


--
-- Name: exam exam_student_person_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.exam
    ADD CONSTRAINT exam_student_person_id_fkey FOREIGN KEY (student_person_id) REFERENCES public.person(person_id);


--
-- Name: person_comment person_comment_blamed_person_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_comment
    ADD CONSTRAINT person_comment_blamed_person_id_fkey FOREIGN KEY (blamed_person_id) REFERENCES public.person(person_id);


--
-- Name: person_comment person_comment_school_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_comment
    ADD CONSTRAINT person_comment_school_id_fkey FOREIGN KEY (school_id) REFERENCES public.school(school_id);


--
-- Name: person person_department_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person
    ADD CONSTRAINT person_department_id_fkey FOREIGN KEY (department_id) REFERENCES public.department(department_id);


--
-- Name: person_school person_school_member_department_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_school
    ADD CONSTRAINT person_school_member_department_id_fkey FOREIGN KEY (member_department_id) REFERENCES public.department(department_id);


--
-- Name: person_school person_school_member_person_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_school
    ADD CONSTRAINT person_school_member_person_id_fkey FOREIGN KEY (member_person_id) REFERENCES public.person(person_id);


--
-- Name: person_school person_school_school_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: lesh
--

ALTER TABLE ONLY public.person_school
    ADD CONSTRAINT person_school_school_id_fkey FOREIGN KEY (school_id) REFERENCES public.school(school_id);

-- Fix such "bigint" as '2+прак'
ALTER TABLE public.person_school ALTER COLUMN courses_needed TYPE varchar;

--
-- PostgreSQL database dump complete
--

