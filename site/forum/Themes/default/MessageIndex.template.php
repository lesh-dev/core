...
function template_main()
{
	...
	<div class="tborder" style="margin-bottom: 3ex; ', $context['browser']['needs_size_fix'] && !$context['browser']['is_ie6'] ? ' width: 100%;' : '', '">
		...
		foreach ($context['boards'] as $board)
		{
			...
			if ($board['new'])
				echo '<img src="', $settings['images_url'], '/on.png" alt="', $txt[333], '" title="', $txt[333], '" />';
			// This board doesn't have new posts, but its children do.
			elseif ($board['children_new'])
				echo '<img src="', $settings['images_url'], '/on2.gif" alt="', $txt[333], '" title="', $txt[333], '" />';
			// No new posts at all! The agony!!
			else
				echo '<img src="', $settings['images_url'], '/off.png" alt="', $txt[334], '" title="', $txt[334], '" />';
	...
	<form action="', $scripturl, '?action=quickmod;board=', $context['current_board'], '.', $context['start'], '" method="post" accept-charset="', $context['character_set'], '" name="quickModForm" id="quickModForm" style="margin: 0;">';
		...
		if (!empty($options['display_quick_mod']) && $options['display_quick_mod'] == 1 && !empty($context['topics'])) {
			...
			makeSubmit('Go', 100, $txt['quick_mod_go'],
				'return document.forms.quickModForm.qaction.value != \'\' &amp;&amp; confirm(\''.$txt['quickmod_confirm'].'\');');
			echo '</td>
					</tr>';
		}
	...
	echo '</select>&nbsp;';
	makeSubmit('JumpToMessageIndex', 100, $txt[161], "if (this.form.jumpto.options[this.form.jumpto.selectedIndex].value) { 
		window.location.href='$scripturl'+this.form.jumpto.options[this.form.jumpto.selectedIndex].value; }");
	echo '</form>';
	echo '<script>ButtonCreate("JumpToMessageIndex", "go");</script>';
	echo	 '</td>
			</tr>
		</table>
	</div>';
