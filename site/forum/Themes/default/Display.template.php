...
function template_main()
{
	...
	$sTextDir = (!$context['right_to_left']) ? 'right' : 'left';
...
<form action="', $scripturl, '?action=quickmod2;topic=', $context['current_topic'], '.', $context['start'], '" method="post" accept-charset="', $context['character_set'], '" name="quickModForm" id="quickModForm" style="margin: 0;" onsubmit="return in_edit_mode == 1 ? modify_save(\'' . $context['session_id'] . '\') : confirm(\'' . $txt['quickmod_confirm'] . '\');">';
	...
	$reply_button = create_button('quote.gif', 145, 'smf240', 'class="postButton"');
	$modify_button = create_button('modify.gif', 66, 17, 'class="postButton"');
	$remove_button = create_button('delete.gif', 121, 31, 'class="postButton"');
	$split_button = create_button('split.gif', 'smf251', 'smf251', 'class="postButton"');
	...
	<tr><td style="padding: 1px 1px 0 1px;">';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
				echo '<p>', $message['member']['post_group'], '</p>';
				echo '<p>', $message['member']['group_stars'], '</p>';

			// Is karma display enabled?  Total or +/-?
			if ($modSettings['karmaMode'] == '1')
				echo '<p>', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</p>';
			elseif ($modSettings['karmaMode'] == '2')
				echo '<p>', $modSettings['karmaLabel'], ' +', $message['member']['karma']['good'], '/-', $message['member']['karma']['bad'], '</p>';

			// Is this user allowed to modify this member's karma?
			if ($message['member']['karma']['allow'])
				echo '<p><a href="', $scripturl, '?action=modifykarma;sa=applaud;uid=', $message['member']['id'],
					';topic=', $context['current_topic'], '.' . $context['start'], ';m=', $message['id'], ';sesc=', $context['session_id'],
					'">', $modSettings['karmaApplaudLabel'], '</a> <a href="', $scripturl, '?action=modifykarma;sa=smite;uid=',
					$message['member']['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';m=', $message['id'],
					';sesc=', $context['session_id'], '">', $modSettings['karmaSmiteLabel'], '</a></p>';

			// Show online and offline buttons?
			if (!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
				echo '<p>', $context['can_send_pm'] ? '<a href="' . $message['member']['online']['href'] . '" title="' .
					$message['member']['online']['label'] . '">' : '', $settings['use_image_buttons'] ?
					'<img src="' . $message['member']['online']['image_href'] . '" alt="' . $message['member']['online']['text'] .
					'" border="0" style="margin-top: 2px;" />' : $message['member']['online']['text'],
					$context['can_send_pm'] ? '</a>' : '', $settings['use_image_buttons'] ? '<span class="smalltext"> ' .
					$message['member']['online']['text'] . '</span>' : '', '</p>';

			// Show the member's gender icon?
			if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '')
				echo '<p>', $txt[231], ': ', $message['member']['gender']['image'], '</p>';

			// Show how many posts they have made.
			echo '<p>', $txt[26], ': ', $message['member']['posts'], '</p>';

			// Show avatars, images, etc.?
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
				echo '<p><div style="overflow: auto; width: 100%;">', $message['member']['avatar']['image'], '</div></p>';

			// Show their personal text?
			if (!empty($settings['show_blurb']) && $message['member']['blurb'] != '')
				echo '<p>', $message['member']['blurb'], '</p>';

			// This shows the popular messaging icons.
			echo '<p>', $message['member']['icq']['link'], ' ',
		...
		echo '</div>
						...
						<td valign="top" width="85%" height="100%">
							<table width="100%" border="0"><tr>
								<td valign="middle" width="50px"><a href="', $message['href'], '"><img src="', $message['icon_url'] . '" alt="" border="0" /></a>';
								$sFastEditButton = '<img src="' . $settings['images_url'] . '/icons/modify_inline.gif" alt="Fast Edit" title="Fast Edit" align="right" id="modify_button_' . $message['id'] . '" class="fastEdit" style="display: none" onclick="modify_msg(\'' . $message['id'] . '\', \'' . $context['session_id'] . '\')" />';
								echo $message['can_modify'] ? $sFastEditButton : '',
								'</td>
								<td valign="middle">
		...
		echo '
								...
								<td align="', $sTextDir, '" class="postButtons">';

		// Can they reply? Have they turned on quick reply?
		
		$sTail = $message['id'].';topic='.$context['current_topic'].'.'.$context['start'];
		$sReplyAction = $scripturl.'?action=post;quote='.$sTail;
		$sEditAction = $scripturl.'?action=post;msg='.$sTail;
		
		$sSID = ';sesc='.$context['session_id'];
		$sNumRep = ';num_replies='.$context['num_replies'];
		$sButtonSpace='&nbsp;&nbsp;&nbsp;';
		if ($context['can_reply'] && !empty($options['display_quick_reply']))
			echo '<a href="',$sReplyAction,$sNumRep,$sSID, '" onclick="doQuote(', $message['id'], ', \'', $context['session_id'], '\'); return false;">', $reply_button, '</a>',$sButtonSpace;

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_reply'])
			echo '<a href="',$sReplyAction,$sNumRep,$sSID,'">',$reply_button,'</a>',$sButtonSpace;

		// Can the user modify the contents of this post?
		if ($message['can_modify'])
			echo '<a href="',$sEditAction,$sSID,'">',$modify_button,'</a>',$sButtonSpace;

		// How about... even... remove it entirely?!
		if ($message['can_remove'])
			echo '<a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], $sSID, '" onclick="return confirm(\'', $txt[154], '?\');">', $remove_button, '</a>',$sButtonSpace;

		// What about splitting it off the rest of the topic?
		if ($context['can_split'])
			echo '<a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '">', $split_button, '</a>',$sButtonSpace;

		// Show a checkbox for quick moderation?
		if (!empty($options['display_quick_mod']) && $options['display_quick_mod'] == 1 && $message['can_remove'])
			echo '<input type="checkbox" name="msgs[]" value="', $message['id'], '" class="check" ', empty($settings['use_tabs']) ? 'onclick="document.getElementById(\'quickmodSubmit\').style.display = \'\';"' : '', ' />',$sButtonSpace;
		
		// Maybe they want to report this post to the moderator(s)?
		if ($context['can_report_moderator'])
			echo '<a href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '">', $txt['rtm1'], '</a>',$sButtonSpace;
		
		
		$sMemberIP = $message['member']['ip'];
		$sDisplayedText='';
		
		$bModerator = $context['can_moderate_forum'];
		
		$bCanSeeIP = $message['can_see_ip'];
		$bLoggedIn = (!$context['user']['is_guest']);
		
		
		if (!empty($sMemberIP)) {
			if ($bModerator || $bCanSeeIP) $sDisplayedText = $sMemberIP;
			else $sDisplayedText = $txt[511]; // "Logged"
		}	
		
		$sImageCode = '<img src="'.$settings['images_url'].'/ip.gif" alt="ip" border="0" title="'.$sDisplayedText.'"/>';
		
		// Show the IP to this user for this post - because you can moderate?
		if ($bModerator) { // href-ed image
			echo '<a href="', $scripturl, '?action=trackip;searchip=', $sMemberIP, '">', $sImageCode, '</a>';
			echo '<a href="', $scripturl, '?action=helpadmin;help=see_admin_ip" onclick="return reqWin(this.href);" class="help">(?)</a>';
		} else {
			if ($bLoggedIn) echo $sImageCode;
			if ($bCanSeeIP || $bLoggedIn) { // Or, should we show it because this is you?
				echo '<a href="', $scripturl, '?action=helpadmin;help=see_member_ip" onclick="return reqWin(this.href);" class="help">(?)</a>';
			}
		}
...
<script language="JavaScript" type="text/javascript"><!-- // --><![CDATA[
	...
	var smf_template_body_edit = \'<div id="error_box" style="padding: 4px; color: red;"></div>',
		'<textarea class="editor" name="message" rows="12" style="width: 94%; margin-bottom: 10px;">%body%</textarea><br />',
		'<input type="hidden" name="sc" value="', $context['session_id'], '" /><input type="hidden" name="topic" value="', $context['current_topic'],
		'" /><input type="hidden" name="msg" value="%msg_id%" /><div style="text-align: center;"><input type="submit" name="post" value="',
		$txt[10], '" onclick="return modify_save(\\\'' . $context['session_id'] . '\\\');"  />&nbsp;&nbsp;',
		$context['show_spellchecking'] ? '<input type="button" value="' . $txt['spell_check'] .
		'" onclick="spellCheck(\\\'quickModForm\\\', \\\'message\\\');" />&nbsp;&nbsp;' : '', '<input type="submit" name="cancel" value="',
		$txt['modify_cancel'], '" onclick="return modify_cancel();" /></div>\';
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 1ex;">
	...
	echo '
				<td valign="top" align="', $sTextDir, '" class="nav"> ', $context['previous_next'], '</td>
		</tr>
</table>';
	...
	echo '
<div class="tborder"><div class="titlebg2" style="padding: 4px;" align="', $sTextDir, '">
	...
	echo '
		...
		// TODO: fix button title, it should be translated!
		makeSubmit('Go', 100, 'Go', "if (this.form.jumpto.options[this.form.jumpto.selectedIndex].value) { 
			window.location.href='$scripturl'+this.form.jumpto.options[this.form.jumpto.selectedIndex].value; }");
	echo '</form>
</div></div>';
	echo "<script>ButtonCreate('Go', 'go');</script>";
	...
	<tr id="quickReplyOptions"', $options['display_quick_reply'] == 2 ? '' : ' style="display: none"', '>
		...
		<td class="windowbg" width="75%" align="center">
			<form action="', $scripturl, '?action=post2" method="post" accept-charset="', $context['character_set'], '" name="postmodify" id="postmodify" onsubmit="submitonce(this);" style="margin: 0;">
				...
				<textarea cols="75" rows="14" style="width: 95%; height: 200px;" name="message" tabindex="1"></textarea><br />';
				// TODO: make these 200px for all languages
				makeSubmit('QuickReplyPost', 200, $txt[105], "return submitThisOnce(this);", 'tabindex="2"');
				makeSubmit('QuickReplyPreview', 200, $txt[507], "return submitThisOnce(this);", 'tabindex="4"');
		if ($context['show_spellchecking'])
			echo '
		...
		echo '<script>
			ButtonCreate("QuickReplyPost", "post.long");
			ButtonCreate("QuickReplyPreview", "preview");
		</script>';
}
echo '<script language="JavaScript" type="text/javascript" src="' . $settings['default_theme_url'] . '/display.js"></script>';
	if ($context['show_spellchecking'])
		echo '
