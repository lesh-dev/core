...
function registerMember(&$regOptions)
{
	...
	// Check registration question
	global $aRegAnswers;
	$sRegAnswer=trim(mb_strtolower($regOptions['regAnswer'],'UTF-8'));
	$nQI=(integer)($regOptions['regQuestionIndex']);
	if ($nQI<0 || $nQI>=count($aRegAnswers)) fatal_lang_error('RegQuestionCheckFailed', false);
	$bAnswerFound=false;
	foreach ($aRegAnswers[$nQI] as $sAns) if ($sRegAnswer==$sAns) $bAnswerFound=true;
	if (!$bAnswerFound) fatal_lang_error('RegQuestionCheckFailed', false);

	// No name?!  How can you register with no name?
	if (empty($regOptions['username'])) fatal_lang_error(37, false);

	// No real name?! Too short real name? You're not a real man?
	if (mb_strlen($regOptions['realName'], 'UTF-8') < 6) fatal_lang_error('RealNameCheckFailed', false);
